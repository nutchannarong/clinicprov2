...
class Ajax_pagination {
    ...
    var $last_link = '&raquo;';
    var $full_tag_open = '<ul class="pagination" style="float: right">';
    ...
    function create_links() {
        ...
        if ( $num_pages == 1 ) {
            $info = '<ul class="custom-pagination" style="float: right">แสดงทั้งหมด ' . $this->total_rows . ' รายการ</ul>';
            return $info;
        }
    ...
    function getAJAXlink( $count, $text ) {
        if ( $this->div == '' ) {
            return '<a class="page-link" href="' . $this->anchor_class . ' ' . $this->base_url . $count . '">' . $text . '</a>';
        }
        ...
        return '<a class="page-link" href="javascript:void(0);" ' . $this->anchor_class . ' onclick="$.post(\'' . $this->base_url . $count . '\', ' . $additional_param . ', function(data){$(\'#' . $this->div . '\').html(data)' . $this->js_rebind . '; }); return false;">' . $text . '</a>';
    }
